...
def get_random_facts_containing_element(anchor_element, anchor_idx, facts, id_to_entity, id_to_relation, num_facts=30):
    containing_facts = []
...
def unmask_fact_batch(client, model, masked_fact, context_facts, anchor_element, anchor_idx, id_to_entity, id_to_relation, num_candidates=10):
...
def gemini_unmask_generator(masked_file, file_dir, output_file=None, model="gemini-3-pro-preview", num_candidates=10):
    client = genai.Client()
    ...
    try:
        for i, masked_fact in enumerate(masked_facts):
            ...
            context_facts = get_random_facts_containing_element(
                anchor_element, anchor_idx, facts, id_to_entity, id_to_relation, num_facts=30
            )
...
if __name__ == "__main__":
    ...
    for dataset_name in datasets:
        ...
        output_filename = f"Few-shotFacts_gemini_targeted_{dataset_name}.txt"
        output_dir = f"./created_data/{dataset_name}"
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, output_filename)
        print(f"Processing dataset: {dataset_name}")        
        gemini_unmask_generator(
