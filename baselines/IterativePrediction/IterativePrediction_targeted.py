...
import json
import random
from mask_utils import load_masked_facts, get_anchor_element_with_idx, load_train_facts
...
def initialize_fact_from_masked(masked_fact, train_graph, anchor_element, anchor_idx):
...
def gibbs_sampling_one_fact(masked_fact, anchor_element, anchor_idx,\
                            model, dataset_obj, train_graph, emb_ents,\
                            emb_rels, init_emb_ents, init_emb_rels, num_iterations=11,\
                            is_sampling=True, batch_num=1):
    fact_len = len(masked_fact)
    
    current_fact = initialize_fact_from_masked(
        masked_fact, train_graph, anchor_element, anchor_idx
    )
...
def generate_with_gibbs(log_name="", exp="maypl", dataset="wd50k-eval", data_dir="../data/",\
    ...
    log_path = f"./logs/{exp}/{dataset}/{log_name}_test_{test_epoch}_iter_targeted.log"
    logger = setup_logger(log_path)
    
    
    if output_file is None:
        output_file = f"./created_data/{dataset}/iter_targeted.txt"
    ...
    with torch.no_grad():
        ...
        pbar = tqdm(enumerate(masked_facts), desc="Iterative Prediction", total=len(masked_facts))
        ...
        for idx, masked_fact in pbar:
            ...
            if anchor_element is None or anchor_idx is None:
