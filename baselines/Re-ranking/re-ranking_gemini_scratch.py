...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "" # Path to your Google Cloud service account JSON file
...
def call_gemini_with_retry(client, model_name, history, question, max_retries=5, logger=None):
    for attempt in range(max_retries):
        try:
            response = client.models.generate_content(
                model=model_name,
                contents=history + [question],
...
def generate_with_gemini(
    ...
    log_path = f"./logs/{exp}/{dataset}/{log_name}_test_{test_epoch}_gemini_scratch_len{fact_len}.log"
    logger = setup_logger(log_path)
    
    
    if output_file is None:
        output_file = f"./created_data/{dataset}/re-ranking_gemini_scratch_len{fact_len}.txt"
        ...
        for i in pbar:
            try:
                ...
                history = [
                    ...
                    types.Content(role="model", parts=[types.Part.from_text(text="Yes")]),
                    types.Content(role="user", parts=[types.Part.from_text(text=prompt2)]),
                    types.Content(role="model", parts=[types.Part.from_text(text="I have received your instructions and the goal statement")]),
                    types.Content(role="user", parts=[types.Part.from_text(text=prompt3)]),
                    types.Content(role="model", parts=[types.Part.from_text(text="I have analyzed the examples provided and will keep thinking without giving feedback.")]),
                ]
                ...
                for loc in range(-1, fact_len):
                    ...
                    if loc + 1 < len(facts_id_temp):
                        ...
                        current_question = types.Content(
                            role="user",
                            parts=[types.Part.from_text(text=prompt4)]
                        )
                        
                        response = call_gemini_with_retry(
                            client, model_name, history, current_question, 
