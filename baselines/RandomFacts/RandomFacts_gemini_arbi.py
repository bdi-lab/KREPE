...
def gemini_naive(masked_file, train_file, file_dir, output_path, model="gemini-3-pro-preview", batch_size=100, num_candidates=10):
    client = genai.Client()
    ...
    while remaining_masked > 0:
        ...
        for masked_fact in current_masked_batch:
            ...
            masked_facts_info.append({
                'masked_str': masked_fact_str,
                'length': len(masked_fact)
            })
        
        masked_list = []
        for i, info in enumerate(masked_facts_info):
            masked_list.append(f"{info['masked_str']}")
        masked_list_str = '\n'.join(masked_list)
...
Your task is to complete the following {current_batch_size} MASKED facts. For EACH masked fact, generate {num_candidates} DIFFERENT hyper-relational facts. Each fact should be unique.
...
"""
        ...
        for i in range(current_batch_size):
            ...
            if all_in_training:
                ...
                for idx, candidate_nl in enumerate(candidates_per_masked[i]):
                    try:
                        candidate_ids = convert_fact_to_ids(candidate_nl, entity_to_id, relation_to_id)
...
if __name__ == "__main__":
    ...
    for dataset_name in datasets:
        ...
        output_filename = f"RandomFacts_gemini_arbi_{dataset_name}.txt"
