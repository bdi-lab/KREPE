...
"""
    ...
    conversation.append({"role": "user", "parts": [{"text": f"{prompt_relation}"}]})
    ...
    relation_nl = response.text.strip()
    relation_id = relation_to_id.get(relation_nl, relation_nl)
    
    conversation.append({"role": "model", "parts": [{"text": relation_nl}]})
...
"""
    ...
    conversation.append({"role": "user", "parts": [{"text": prompt_tail}]})
    ...
    tail_nl = response.text.strip()
    tail_id = entity_to_id.get(tail_nl, tail_nl)
    
    conversation.append({"role": "model", "parts": [{"text": tail_nl}]})
...
"""
        ...
        conversation.append({"role": "user", "parts": [{"text": prompt_q_rel}]})
...
"""
        ...
        conversation.append({"role": "user", "parts": [{"text": prompt_q_val}]})
        ...
        q_value_nl = response.text.strip()
        q_value_id = entity_to_id.get(q_value_nl, q_value_nl)
        current_fact_nl.append(q_value_nl)
        
        conversation.append({"role": "model", "parts": [{"text": q_value_nl}]})
...
def gemini_sequential(num, fact_length, file_dir, output_file=None, model="gemini-3-pro-preview"):
    client = genai.Client()
...
if __name__ == "__main__":
    ...
    for dataset_name, fact_configs in datasets.items():
        output_filename = f"Autoregressive_gemini_scratch_{dataset_name}.txt"
